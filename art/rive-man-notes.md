...
The file was exported on 2026-09-07 and is wired into the React scene. See [shared runtime implementation notes](../public/rive/README.md) for exact names, property access, code examples, animation control limits, and export validation.
...
Layer 2 contains the `Anime Appearance` 1D blend state. Timeline 6 is its corporate endpoint (0), and Timeline 7 its anime endpoint (1). Timeline 5 contains the editable full morph. The endpoint timelines use short work areas near the corresponding ends of that morph; if editing its animation keys, update both endpoint copies too.
